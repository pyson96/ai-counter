"""SOLIDER Swin-Transformer backbone.

Vendored verbatim (minus the unused `mmcv`/`cv2` imports) from
https://github.com/tinyvision/SOLIDER-REID  ->  model/backbones/swin_transformer.py
Kept in its own file because it is third-party model-definition code; all of the
project's own logic lives in models.py / pipeline.py / main.py.
"""
import warnings
